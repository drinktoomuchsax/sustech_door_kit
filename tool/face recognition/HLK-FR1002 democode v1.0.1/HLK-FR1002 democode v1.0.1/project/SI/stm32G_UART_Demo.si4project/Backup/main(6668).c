...
uint8_t g_fUART1_Byte[64];
uint8_t uart_get_flag = 0;

UART_HandleTypeDef huart1;
UART_HandleTypeDef huart2;
uint8_t compare_bytes[24] = {0x58, 0x53, 0x31, 0x30, 0x30, 0x31, 0x5F,
...
int main(void)
{
  ...
  while (1)
  {
	...
	if(1 == uart_get_flag)
		{
			int i = 0;
			uint8_t data = ring_buff[ring_buff_index];
			for (; i < 24; i++)
			{
				if ( data != compare_bytes[i])
				{
