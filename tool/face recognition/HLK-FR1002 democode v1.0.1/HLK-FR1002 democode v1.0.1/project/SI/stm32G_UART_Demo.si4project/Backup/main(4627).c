...
int State_of_Enrollment(P_USER p_user, uint8_t dir);
...
void Enrollment(UART_HandleTypeDef huart, P_USER p_user, uint8_t dir, uint8_t enrollment_pattern)
{
	...
	HAL_UART_Transmit(&huart, pEnrollCmd, 8, 1000);
	HAL_UART_Receive_IT(&huart, &g_fUART1_Byte, 1);
	//----------------------
	int result;
	int t = 0;
	if (enrollment_pattern == 1)
	{
		while (1)
		{
			...
			result = State_of_Enrollment(p_user, face_dir[dir]);
			if(result != -1)
			{
				if(result == 0)
				{
				...
				{
					printf("enroll success, the user id: %d%d, ", p_user->id[0], p_user->id[1]);
