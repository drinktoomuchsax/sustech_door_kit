...
uint8_t version_bytes[24];
	/*= {0x58, 0x53, 0x31, 0x30, 0x30, 0x31, 0x5F,
							 0x56, 0x31, 0x2E, 0x30, 0x2E, 0x38, 0x5F,
							 0x31, 0x2E, 0x30, 0x2E, 0x30, 0x2E, 0x30, 
							 0x37, 0x32, 0x38};*/
UART_HandleTypeDef huart1;
UART_HandleTypeDef huart2;
/* USER CODE END PD */
...
int GetFirmwareVersion(UART_HandleTypeDef huart)
{
	char pCmd1[6] = { 0xEF, 0xAA, 0x30, 0x00, 0x00, 0x30 };
	HAL_UART_Transmit(&huart, (uint8_t *)pCmd1, 6, 1000);
	HAL_UART_Receive_IT(&huart1, &g_fUART1_Byte, 64);
	return 0;
}
...
uint8_t DataStorage(uint8_t data){
	if (index_2 ==24){
		index_2 = 0;
		return 2;
		}
	version_bytes[index_2] = data;
	index_2++;
	return 1;
}

void UART_Compare(uint8_t data)
{
	...
	switch(state)
	{
		case 0:
			...
			current_step = 0;
		case 1:
			if (current_step >=6 && current_step <=30){
...
void USART1_IRQHandler(void)
{
  ...
  /* data storage*/
    ...
    UART_Compare(ring_buff[ring_buff_index]);
    ++ ring_buff_index;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	DEBUG_Printf("start", version_bytes, 24);
