...
void State_of_Match(P_USER p_user);
...
void Enrollment(UART_HandleTypeDef huart, P_USER p_user, uint8_t dir, 	uint8_t enrollment_pattern)
{
...
void Match(UART_HandleTypeDef huart, P_USER p_user)
{
	...
	function.cmd = 0x12;
	uint8_t nCycleTime = (0x03+1) * 1000 / NORM_CMD_TIME_OUT;
	HAL_UART_Transmit(&huart, pCmd1, 8, 1000);
	HAL_UART_Receive_IT(&huart, &g_fUART1_Byte, 1 );
	uint8_t t = 0;
	while (t<5)
	{
		for(uint8_t i = 0; i < nCycleTime; ++i)
			if(data_recieve_finished)
				...
				Delay_ms(NORM_CMD_TIME_OUT);
		uint8_t result = State_of_Match(p_user);
		if(result == 1)
		{
			printf("match success, id: %d, name: %s", p_user->id, p_user->name);
			break;
		}
		else if (result == 0)
		{
			printf("match fail, cannot find user");
			break;
		}
		else
			data_recieve_finished = 0;
...
uint8_t State_of_Match(P_USER p_user)
{
	if(function.data_length[1] == 0x26 && function.cmd == 0x12)
	{
		if(function.state == 0x00)
		{
