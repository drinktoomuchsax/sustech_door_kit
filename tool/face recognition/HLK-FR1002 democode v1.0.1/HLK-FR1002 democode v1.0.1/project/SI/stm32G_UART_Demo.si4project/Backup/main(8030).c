...
uint32_t DeleteID(UART_HandleTypeDef huart, uint8_t id)
{
...
void Enrollment(UART_HandleTypeDef huart, P_USER p_user, uint32_t dir)
{
	...
	if (p_user->enrollment_pattern == 5) {
		pEnrollCmd[39] = 0x00;
	}
	else if (p_user->enrollment_pattern == 1) {
		pEnrollCmd[39] = 0x01;
	}
