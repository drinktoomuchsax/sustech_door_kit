...
void Enrollment(UART_HandleTypeDef huart, P_USER p_user, uint8_t dir, uint8_t enrollment_pattern)
{
	...
	if (enrollment_pattern == 1)
	{
		while (1)
		{
			...
			result = State_of_Enrollment(p_user, face_dir[dir], enrollment_pattern);
			if(result != -1)
			{
				if(result == 0)
				{
		...
		while (1)
		{
			...
			if(result != -1)
			{
				...
				else
				{
					...
					{
						printf("enroll success, please look %s", str_dir[dir]);
						break;
					}
				}
			}
...
int State_of_Enrollment(P_USER p_user, uint8_t dir, uint8_t enrollment_pattern)
{
	if(function.data_length[1] == 0x05)
		...
		{
			if(enrollment_pattern == 1 || dir == 0x02)
			{
				p_user->id[0] = function.data[0];
				p_user->id[1] = function.data[1];
			}
