...
uint8_t GetDataLength(uint8_t data, RECV_MSG recv_msg){
	static uint32_t index_3 = 0;
	recv_msg.data_length[index_3] = data;
	if (index_3 == 0)
	{
		index_3++;
		return GET_LENGTH;
		...
		index_3 = 0;
		length = recv_msg.data_length[0]*256 + recv_msg.data_length[1];
		*recv_msg = (RECV_MSG *)malloc(sizeof(RECV_MSG)*length);
		return GET_CMD;
	}
	else
	{
...
uint8_t VerifyDataCMD(uint8_t data, RECV_MSG recv_msg){
	if (data == recv_msg.cmd)
		return GET_STATE;
	else{
		current_step = -1;
		return GET_HEAD;
...
uint8_t GetModuleState(uint8_t data, RECV_MSG recv_msg){
	recv_msg.state = data;
	if (data == 0x00)
		return GET_DATA;
	else{
...
uint8_t DataStorage(uint8_t data, RECV_MSG recv_msg){
	
	recv_msg.data[index_2] = data;
	++index_2;
	if (index_2 > length)
	{
		index_2 = 0;
...
uint8_t VerifyParity(uint8_t data, RECV_MSG recv_msg)
{
	uint8_t nParityCheck = 0;
	nParityCheck ^= recv_msg.data_length[0];
	nParityCheck ^= recv_msg.data_length[1];
	nParityCheck ^= recv_msg.cmd;
	nParityCheck ^= recv_msg.state;

	for (uint32_t i = 0; i < length ; i++)
		nParityCheck ^= recv_msg.data[i];
...
void UART_Compare(uint8_t data, RECV_MSG recv_msg)
{
	switch(state)
	{
		case GET_HEAD:
		...
		case GET_LENGTH:
			if (current_step > 2)
				state = GetDataLength(data, recv_msg);
			current_step++;
			break;
		case GET_CMD:
			if(current_step > 4)
				state = VerifyDataCMD(data, recv_msg);
			current_step++;
			break;
		case GET_STATE:
			if(current_step > 5)
				state = GetModuleState(data, recv_msg);
			current_step++;
			break;
		case GET_DATA:
			if (current_step > 6 && current_step <= 32){
				state = DataStorage(data, recv_msg);
			}
			current_step++;
			break;
		case GET_PARITY:
			...
			if(current_step > length)
				state = VerifyParity(data, recv_msg);
		case END_RECIEVE:
			current_step = 0;
...
uint8_t Check_UartIsOK(void)
{
	...
	//如果flag不为1则表示没触发中断，没有数据接收
	//同时也表示如果一直有数据发送过来则会一直让其处于等待数据接收完毕的状态
	if(uart_get_flag)
	{
		uart_get_flag = 0;
		calc_T = 0xFFFF;
