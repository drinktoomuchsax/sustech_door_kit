...
{    
  ...
  for (num = 0; num < delay_us && !IT_Button; num++)
  {
    t = 11;
    while (t != 0 && !IT_Button)
    {
...
{    
  volatile unsigned int num;
  for (num = 0; num < delay_ms && !IT_Button; num++)
  {
    Delay_us(1000);
  }
}
...
void Receive_Interruption(uint8_t g_fUART1_Byte, P_RECV_MSG p_function)
{
...
int State_of_PowerDown(P_RECV_MSG p_function)
{
...
int ML_API_PowerDown(P_RECV_MSG p_function) 
{
	...
	if(t>=nCycleTime)	
		...
		return State_of_PowerDown(p_function);
	
}

/************************************************************************
function name: ML_API_PresentationMode
args:P_RECV_MSG p_function, uint8_t is_enable
************************************************************************/

int State_of_PresentationMode(P_RECV_MSG p_function)
{
	if(p_function->state == 0x00)
		return 1;
	else
...
int ML_API_PresentationMode(P_RECV_MSG p_function, uint8_t is_enable) 
{
	uint8_t pCmd1[7] = { 0xEF, 0xAA, 0xFE, 0x00, 0x01, 0x00, 0x00};
	p_function->cmd = 0xFE;
	...
	pCmd1[6] = GetParityCheck(pCmd1, sizeof(pCmd1));
	...
	if(t>=nCycleTime)
		...
		return State_of_PresentationMode(p_function);
	
	
}

/************************************************************************
function name: ML_API_GetModuleState
args:P_RECV_MSG p_function
************************************************************************/

int State_of_Module(P_RECV_MSG p_function)
{
	if(p_function->state == 0x00)
...
int ML_API_GetModuleState(P_RECV_MSG p_function)

{
	uint8_t pCmd1[6] = { 0xEF, 0xAA, 0x23, 0x00, 0x00, 0x23 };
	p_function->cmd = 0x23;
	...
	if(t>=nCycleTime)
		...
		return State_of_Module(p_function);
...
int State_of_ResetAndStandby(P_RECV_MSG p_function)
{
	if(p_function->state == 0x00)
		return 1;
	else
...
int ML_API_ResetAndStandby(P_RECV_MSG p_function) 
{
	uint8_t pCmd1[6] = { 0xEF, 0xAA, 0x23, 0x00, 0x00, 0x23 };
	p_function->cmd = 0x23;
	...
	if(t>=nCycleTime)
		...
		return State_of_ResetAndStandby(p_function);
...
int State_of_GetAllUserID(P_RECV_MSG p_function)
{
	if(p_function->state == 0x00)
		return p_function->data[0];
	else
...
int ML_API_GetAllUserID(P_RECV_MSG p_function) 
{
...
************************************************************************/
int State_of_DeleteID(uint8_t id, P_RECV_MSG p_function)
{
...
int ML_API_DeleteID(uint8_t id, P_RECV_MSG p_function)
{
	...
	if(t>=nCycleTime)
		...
		return State_of_DeleteID(id, p_function);
...
args:P_RECV_MSG p_function
************************************************************************/
int State_of_DeleteAll(P_RECV_MSG p_function)
{
	if(p_function->state == 0x00)
		return 1;
	else
...
int ML_API_DeleteAll(P_RECV_MSG p_function) 
{
	...
	if(t>=nCycleTime)
		...
		return State_of_DeleteAll(p_function);
...
int ML_API_GetFirmwareVersion(P_RECV_MSG p_function)
{
...
args:P_RECV_MSG p_function
************************************************************************/
int ML_API_GetUUID(P_RECV_MSG p_function)
{
	uint8_t pCmd1[6] = {0xEF, 0xAA, 0x93, 0x00, 0x00, 0x93};
	p_function->cmd = 0x93;
...
************************************************************************/
int State_of_Enrollment_0x26(P_RECV_MSG p_function, P_USER p_user, uint8_t dir, uint8_t enrollment_pattern)
{
...
int ML_API_Enrollment_0x26(P_RECV_MSG p_function, P_USER p_user, uint8_t dir, uint8_t enrollment_pattern)
{
	...
	if (enrollment_pattern == 1)
	{
		while (1)
		{
			...
			result = State_of_Enrollment_0x26(p_function, p_user, pAns[dir], enrollment_pattern);
			if(result != -1)
			{
				if(result == 0)
				{
...
args:P_RECV_MSG p_function, P_USER p_user, uint8_t dir, uint8_t enrollment_pattern
************************************************************************/
int State_of_Enrollment_0x13(P_RECV_MSG p_function, P_USER p_user, uint8_t dir)
{
	if(p_function->data_length[1] == 0x05)
		if(p_function->state == 0x00 && p_function->data[2] == dir)
		{
...
int ML_API_Enrollment_0x13(P_RECV_MSG p_function, P_USER p_user, uint8_t dir)
{
	uint8_t pEnrollCmd[] = { 0xEF, 0xAA, 0x13, 0x00, 0x23, 0x00, 0x00, 0x00, 0x00, 0x00, 
							 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
							 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
							 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x0A, 0x3B };
	p_function->cmd = 0x13;
	pEnrollCmd[5] = p_user->isAdm;
	...
	pEnrollCmd[39] = nTimeOut;
	pEnrollCmd[40] = GetParityCheck(pEnrollCmd, sizeof(pEnrollCmd));
	...
	while (1)
	{
		...
		result = State_of_Enrollment_0x13(p_function, p_user, pAns[dir]);
